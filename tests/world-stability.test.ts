...
const state={x:tree.x-2,y:tree.y,z:tree.z,vx:0,vy:0,vz:0,yaw:-Math.PI/2,pitch:0,grounded:true,lastLandingSpeed:0};
...
for(const house of BUILDINGS.filter(b=>b.style==='house')) {
 const p={x:(house.x+Math.floor(house.w/2)+.5)*3,y:.25,z:house.z*3-2,vx:0,vy:0,vz:0,yaw:0,pitch:0,grounded:true,lastLandingSpeed:0};
