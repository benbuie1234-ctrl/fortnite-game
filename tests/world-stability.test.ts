...
assert.ok(BUILDINGS.filter(b=>b.style==='cabin'&&b.base===0).length>=4);
...
for(const house of BUILDINGS.filter(b=>b.style==='house')) {
 const p={x:(house.x+Math.floor(house.w/2)+.5)*TILE,y:.25,z:house.z*TILE-2,vx:0,vy:0,vz:0,yaw:0,pitch:0,grounded:true,lastLandingSpeed:0};
 for(let i=0;i<24;i++)stepPlayer(p,{seq:i,moveX:0,moveZ:1,yaw:0,pitch:0,buttons:0,slot:2},world);
 assert.ok(p.z>house.z*TILE+1,'house entrance remains walkable');
 assert.ok(!PROPS.some(prop=>Math.abs(prop.x-(house.x+Math.floor(house.w/2)+.5)*TILE)<1&&Math.abs(prop.z-house.z*TILE)<1),'door is clear of furnishing');
}
